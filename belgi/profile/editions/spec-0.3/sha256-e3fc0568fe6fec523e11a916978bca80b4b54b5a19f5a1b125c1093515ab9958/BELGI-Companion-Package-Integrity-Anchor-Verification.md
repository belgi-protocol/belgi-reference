...
- BELGI - Part 1: Core semantic model, Version 0.3, 2026-07-19
- BELGI - Part 2: Claim carriers and replay package, Version 0.3, 2026-07-19
- BELGI - Part 3: Profiles and companion specifications, Version 0.3, 2026-07-19
...
stable opaque identifier naming one package-integrity-anchor verification method across its defining editions
...
This companion specification depends on the BELGI part draft versions identified in Clause 2.
...
This `0.3` draft succeeds `0.2`. The predecessor identifier `urn:belgi:companion:package-integrity-anchor-verification` identifies only the predecessor family editions and is not an alias for the identifier in 6.1. This companion specification declares no backward compatibility, forward compatibility, identifier substitution, or replay substitution with its predecessor.
...
The package-integrity anchor shall carry both the stable signature-algorithm identifier and the defining-source designator of the exact edition that defines that algorithm's use by this method.

That algorithm shall be interpreted only according to the exact-edition surface fixed by this companion specification and the BELGI exact-edition dependencies on which it relies.
...
The anchor shall carry the verification-method identifier in 8.1 separately from the defining-source designator of the exact edition that defines that method. It shall carry the Ed25519 algorithm identifier in 8.1 separately from the defining-source designator of the exact edition that defines Ed25519 use by this method.
...
The signature algorithm is pure Ed25519 as specified by RFC 8032. Ed25519ctx and Ed25519ph are not admitted. The message is the exact preserved octet sequence of the designated package-integrity manifest; no prehash, context string, alternate canonicalization, normalization, or reconstruction is applied.

The 64 decoded signature octets are interpreted as the RFC 8032 `R` encoding followed by the scalar `S`. Verification shall apply the pure Ed25519 verification procedure and encoding checks in RFC 8032, including valid decoding of the public key and `R`, the canonical scalar requirement `S < L`, and the RFC 8032 verification equation.
