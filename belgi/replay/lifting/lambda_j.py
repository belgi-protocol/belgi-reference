...
def lift_judged_object(
    *,
...
) -> LiftingTrace[JudgedParsedT, JudgedResolvedT, JudgedT]:
    root_reference = root_member.canonical_reference
    if root_reference is None:
        raise ParseFailureError(
            message="Judged-object root member lacks a canonical reference."
        )
    try:
        root_bytes = read_member_bytes(package=package, member=root_member)
    except Exception as exc:
        raise ParseFailureError(
            message=f"Could not read judged-object root member {root_member.member_name!r}.",
            related_reference=root_reference,
        ) from exc

    try:
        parsed = replay_context.judged_lifting.parse(
        ...
        raise ParseFailureError(
            message=f"Judged-object carrier parse failed: {exc}",
            related_reference=root_reference,
        ) from exc
    ...
    try:
        resolved = replay_context.judged_lifting.resolve(
            parsed=parsed,
        ...
        raise ResolveFailureError(
            message=f"Judged-object carrier resolution failed: {exc}",
            related_reference=root_reference,
        ) from exc

    try:
        induced = replay_context.judged_lifting.induce(
            resolved=resolved,
            claim_record=claim_record,
        )
        ...
        raise InduceFailureError(
            message=f"Judged-object induction failed: {exc}",
            related_reference=root_reference,
        ) from exc
    return LiftingTrace(
        parsed=ParseRecord(
